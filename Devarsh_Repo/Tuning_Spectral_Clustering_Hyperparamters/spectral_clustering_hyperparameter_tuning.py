...
# def adaptive_clustering(G, method, threshold=60, max_attempts=10, resolution=0.1,
#                         num_clusters=2):
#     # placing a limit on maximum fragment size
#     # resolution = 0.1         # Initial value for Louvain
#     # num_clusters = 2         # Initial number for Spectral    (replace!)
#     communities = []
...
#     fragment_list_LM = adaptive_clustering(Graph_LM, "LM")  # allows to impose a limit on avg. fragment length
...
#         raise ValueError(
#             "Expected exactly one base file (without _Frag_LM_ or _Frag_SC_), found: " + str(base_candidates))
...
def get_num_clusters_list(graph_size, threshold_min, threshold_max):
    k_min = math.floor(graph_size / threshold_max)
    k_max = math.floor(graph_size / threshold_min)
...
def auto_tune_clustering(File_Path, max_runs=100, target_diff=0.01):
    ...
    #resolutions = [round(0.1 + (3 - 0.1) * i / 29, 3) for i in range(30)]
    threshold_min = 20
    threshold_max = 90
    G = MakeGraph(File_Path)
    graph_size = len(G.nodes)
    num_clusters_list = get_num_clusters_list(graph_size, threshold_min, threshold_max)
    #num_clusters_list = list(range(2, 16))
    ...
    #min_diff_for_thershold_LM = float('inf')
    min_diff_overall_SC = float('inf')
    #min_diff_overall_LM = float('inf')
    #Found_Candidate_LM = False
    ...
    best_params_SC = {}
        ...
        # ### --- Louvain Clustering --- ###
        # if not Found_Candidate_LM:
        #     for resolution in resolutions:
        #         run_count_LM += 1

        #         G = MakeGraph(File_Path)
        #         lm_frags = adaptive_clustering(G, "LM", threshold=threshold, max_attempts=10, resolution=resolution)
        #         cut_edges_LM = visualisation(lm_frags, G, "LM")
        #         fragments_LM = get_fragment_atom_lists_with_caps(File_Path, lm_frags, cut_edges_LM)
        #         file_list_LM = write_xyz_fragments(fragments_LM, File_Path, "LM")

        #         lm_energy, lm_time = run_fragments_joblib(file_list_LM[1:], cut_edges_LM)
        #         diff = abs(base_energy - lm_energy)
        ...
        #                 "energy": lm_energy,
        #                 "energy_diff": diff}
        #             Found_Candidate_LM = True
        #             break

        #         if diff < min_diff_for_thershold_LM:
        #             min_diff_for_thershold_LM = diff
        #             temp_params_LM = {
        #                 "Model": "LM",
        #                 "threshold": threshold,
        #                 "resolution": resolution,
        ...
        #                 "energy_diff": diff
        #             }
        #         if run_count_LM == max_runs:
        #             print("Reached max runs limit.")
        #             break

        #         # Clean up .xyz files
        #         for file in file_list_LM[1:]:
    ...
    for num_clusters in num_clusters_list:
        ...
        # elif min_diff_for_thershold_LM < min_diff_overall_LM:
        #     min_diff_overall_LM = min_diff_for_thershold_LM
        #     best_params_LM = {
        #         "Model": "LM",
        #         "threshold": temp_params_LM["threshold"],
        #         "resolution": temp_params_LM["resolution"],
        #         "energy": temp_params_LM["energy"],
        #         "energy_diff": temp_params_LM["energy_diff"]}


    # elif min_diff_for_thershold_SC < min_diff_overall_SC:
    #     min_diff_overall_SC = min_diff_for_thershold_SC
    #     best_params_SC = {
    #         "Model": "SC",
    #         "threshold": temp_params_SC["threshold"],
    #         "Number of Clusters": temp_params_SC["Number of Clusters"],
    #         "energy": temp_params_SC["energy"],
    #         "energy_diff": temp_params_SC["energy_diff"]}
    # if not Found_Candidate_LM:
    #     print("Didn't Converge, ", best_params_LM)
    #     file_exists = os.path.isfile("LM_Not_Converged.csv")
    #     with open("LM_Not_Converged.csv", mode='a', newline='') as file:
    #         writer = csv.DictWriter(file, fieldnames=best_params_LM.keys())
    #         if not file_exists:
    #             writer.writeheader()
    #         writer.writerow(best_params_LM)
    ...
    # if Found_Candidate_LM:
    #     print("LM Converged ", best_params_LM)
    #     file_exists = os.path.isfile("LM_Converged.csv")
    #     with open("LM_Converged.csv", mode='a', newline='') as file:
    #         writer = csv.DictWriter(file, fieldnames=best_params_LM.keys())
    #         if not file_exists:
    #             writer.writeheader()
    #         writer.writerow(best_params_LM)
