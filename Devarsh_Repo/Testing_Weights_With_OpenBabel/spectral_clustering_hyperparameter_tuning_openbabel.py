...
def Add_Weights(Adj_Matrix, File_Path):
    ...
    for i in range(n):
        ...
        ideal = ideal_degrees.get(element, num_of_bonds)  # If atom has ideal bonds, no need to adjust weights
        if num_of_bonds >= ideal:
            continue

        for j in range(n):       # Iterate through bonded neighbors
            if Adj_Matrix[i][j] == 1:
                neighbor = atoms[j]
                ...
                if missing_i > 0 and missing_j > 0:      # Define weight logic (more missing = stronger, harder to cut)
                    weight = 3
                ...
                Adj_Matrix[i][j] = weight  # Assign weight symmetrically
                Adj_Matrix[j][i] = weight
    # To check if weights were added
    # indices = np.argwhere(Adj_Matrix > 1)
    # for i, j in indices:
    #     print(f"Edge ({i}, {j}) has weight {Adj_Matrix[i][j]}")

    return Adj_Matrix


def MakeAdjecencyMatrix(File_Path):
    Adj_Matrix = get_bond_order_adjacency_openbabel(File_Path)
    return Adj_Matrix
    # mg = MolGraph()
    # mg.read_xyz(File_Path)
    # graph = mg.to_networkx()
    # adj_graph = adjacency_matrix(graph).toarray()       # makes it editable
    # adj_graph = Add_Weights(adj_graph,File_Path)
    # return adj_graph  # Provides Adj_Matrix to work with
...
def Run_Spectral_Clustering(File_Path):
    Matrix_SC = MakeAdjecencyMatrix(File_Path)  # Receives Adj_Matrix from xyz file
...
def auto_tune_clustering(File_Path, max_runs=100, target_diff=0.01):
    ...
    for num_clusters in num_clusters_list:
        ...
        sc_frags = spectral_cut2(A, num_clusters)
